...
type ReportingEmergencyScreenRouteProp = RouteProp<
  RootStackParamList,
  'ReportingEmergencyScreen'
>;

type ReportingEmergencyScreenNavigationProp = StackNavigationProp<
  RootStackParamList,
  'ReportingEmergencyScreen'
>;
...
export default function ReportingEmergencyScreen() {
  const insets = useSafeAreaInsets();
  const backgroundHeight = height * 2;
  const route = useRoute<ReportingEmergencyScreenRouteProp>();
  const navigation = useNavigation<ReportingEmergencyScreenNavigationProp>();
  const [tapCount, setTapCount] = useState(0);
  const { emergencyData, location } = route.params;
  ...
  useEffect(() => {
    if (tapCount === 5) {
      navigation.navigate('ReportVerifiedScreen', {
        emergencyData: emergencyData,
        location: location,
      });
      setTapCount(0);
    }
  }, [tapCount, navigation, emergencyData, location]);
